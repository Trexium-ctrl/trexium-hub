  ...
  return (
    <div className="p-6 max-w-7xl mx-auto">
      <PageHeader
        ...
        action={<Button onClick={() => { setEditingLead(null); setDialogOpen(true); }} className="bg-[#00F0FF] hover:bg-[#00C8D6] text-white"><Plus className="w-4 h-4 mr-1.5" />Add Lead</Button>}
      />
      ...
      <div className="flex flex-col sm:flex-row gap-2 mb-4">
        <div className="relative flex-1">
          ...
          <Input value={search} onChange={e => setSearch(e.target.value)} placeholder="Search company, contact, email..." className="pl-9 bg-[#0D0D12] border-[#1E1E26] text-[#FFFFFF] placeholder:text-[#3A3A45]" />
        </div>
        ...
        <Select value={partnerFilter} onValueChange={setPartnerFilter}>
          <SelectTrigger className="w-full sm:w-40 bg-[#0D0D12] border-[#1E1E26] text-[#FFFFFF]"><SelectValue /></SelectTrigger>
          <SelectContent className="bg-[#0D0D12] border-[#1E1E26]">
            <SelectItem value="all" className="text-[#FFFFFF] focus:bg-[#161620]">All partners</SelectItem>
            <SelectItem value="Owner" className="text-[#FFFFFF] focus:bg-[#161620]">Owner</SelectItem>
            <SelectItem value="Partner" className="text-[#FFFFFF] focus:bg-[#161620]">Partner</SelectItem>
          </SelectContent>
        </Select>
        <Select value={sortField} onValueChange={setSortField}>
          <SelectTrigger className="w-full sm:w-48 bg-[#0D0D12] border-[#1E1E26] text-[#FFFFFF]"><SelectValue /></SelectTrigger>
          <SelectContent className="bg-[#0D0D12] border-[#1E1E26]">
            <SelectItem value="next_follow_up_date" className="text-[#FFFFFF] focus:bg-[#161620]">Next follow-up</SelectItem>
            <SelectItem value="priority" className="text-[#FFFFFF] focus:bg-[#161620]">Priority</SelectItem>
            <SelectItem value="estimated_value" className="text-[#FFFFFF] focus:bg-[#161620]">Est. value</SelectItem>
          </SelectContent>
        </Select>
      </div>
      ...
      {filtered.length === 0 ? (
        ...
        <div className="space-y-2">
          {filtered.map(lead => {
            ...
            return (
              <div key={lead.id} className="bg-[#0D0D12] border border-[#1E1E26] rounded-xl p-4 hover:border-[#3A3A45] transition-colors">
                <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-3">
                  <div className="flex items-start gap-3 min-w-0 flex-1">
                    <div className="w-10 h-10 rounded-lg bg-[#161620] flex items-center justify-center flex-shrink-0">
                      <Building2 className="w-5 h-5 text-[#A0A0A0]" />
                    </div>
                    <div className="min-w-0 flex-1">
                      <div className="flex items-center gap-2 flex-wrap">
                        <p className="text-sm font-semibold text-[#FFFFFF]">{lead.company_name}</p>
                      ...
                      <div className="flex items-center gap-4 mt-1 text-xs text-[#A0A0A0] flex-wrap">
                      ...
                      {lead.notes && <p className="text-xs text-[#A0A0A0] mt-1 line-clamp-1">{lead.notes}</p>}
                    </div>
                  </div>
                  <div className="flex items-center gap-1.5 flex-shrink-0">
                    {lead.phone && (
                      <a href={`tel:${lead.phone}`} className="w-8 h-8 rounded-lg bg-[#161620] hover:bg-[#1E1E26] flex items-center justify-center" title="Call">
                        <Phone className="w-3.5 h-3.5 text-[#A0A0A0]" />
                      </a>
                    )}
                    {lead.email && (
                      <a href={`mailto:${lead.email}`} className="w-8 h-8 rounded-lg bg-[#161620] hover:bg-[#1E1E26] flex items-center justify-center" title="Email">
                    ...
                    {lead.status !== 'Won' && (
                      <button onClick={() => setConvertLead(lead)} className="h-8 px-2.5 rounded-lg bg-[#00F0FF]/10 hover:bg-[#00F0FF]/20 text-[#00F0FF] text-xs font-medium flex items-center gap-1" title="Convert to customer">
                        <ArrowRight className="w-3.5 h-3.5" /> Convert
                      </button>
                    )}
                    <button onClick={() => { setEditingLead(lead); setDialogOpen(true); }} className="w-8 h-8 rounded-lg bg-[#161620] hover:bg-[#1E1E26] flex items-center justify-center" title="Edit">
                      <Pencil className="w-3.5 h-3.5 text-[#A0A0A0]" />
                    </button>
                    <button onClick={async () => { await base44.entities.Lead.delete(lead.id); loadLeads(); }} className="w-8 h-8 rounded-lg bg-[#161620] hover:bg-red-500/20 flex items-center justify-center" title="Delete">
                      <Trash2 className="w-3.5 h-3.5 text-[#A0A0A0] hover:text-red-400" />
                    </button>
                  </div>
                </div>
              </div>
      ...
      <Dialog open={!!convertLead} onOpenChange={() => setConvertLead(null)}>
        <DialogContent className="bg-[#0D0D12] border-[#1E1E26] text-[#FFFFFF] max-w-md">
          <DialogHeader>
            <DialogTitle className="text-[#FFFFFF]">Convert to Customer</DialogTitle>
          </DialogHeader>
          <p className="text-sm text-[#A0A0A0]">This will create a new customer from <span className="text-[#FFFFFF] font-medium">{convertLead?.company_name}</span> and mark the lead as Won.</p>
          <DialogFooter>
            <Button variant="ghost" onClick={() => setConvertLead(null)} className="text-[#A0A0A0]">Cancel</Button>
            <Button onClick={handleConvert} className="bg-[#00F0FF] hover:bg-[#00C8D6] text-white">Convert</Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
    </div>
  ...
  return (
    <Dialog open={open} onOpenChange={onClose}>
      <DialogContent className="bg-[#0D0D12] border-[#1E1E26] text-[#FFFFFF] max-w-2xl max-h-[90vh] overflow-y-auto">
        <DialogHeader><DialogTitle className="text-[#FFFFFF]">{editing ? 'Edit Lead' : 'Add Lead'}</DialogTitle></DialogHeader>
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
          <div><Label className="text-[#A0A0A0] text-xs">Company Name *</Label><Input value={form.company_name || ''} onChange={e => set('company_name', e.target.value)} className="mt-1 bg-[#050508] border-[#1E1E26] text-[#FFFFFF]" /></div>
          <div><Label className="text-[#A0A0A0] text-xs">Industry</Label><Input value={form.industry || ''} onChange={e => set('industry', e.target.value)} className="mt-1 bg-[#050508] border-[#1E1E26] text-[#FFFFFF]" /></div>
          ...
          <div><Label className="text-[#A0A0A0] text-xs">Current Website Quality</Label>
            <Select value={form.current_website_quality || 'None'} onValueChange={v => set('current_website_quality', v)}>
              ...
              <SelectContent className="bg-[#0D0D12] border-[#1E1E26]">{QUALITIES.map(q => <SelectItem key={q} value={q} className="text-[#FFFFFF] focus:bg-[#161620]">{q}</SelectItem>)}</SelectContent>
            </Select>
          </div>
          ...
          <div><Label className="text-[#A0A0A0] text-xs">Location</Label><Input value={form.location || ''} onChange={e => set('location', e.target.value)} className="mt-1 bg-[#050508] border-[#1E1E26] text-[#FFFFFF]" /></div>
          <div><Label className="text-[#A0A0A0] text-xs">Source</Label><Input value={form.source || ''} onChange={e => set('source', e.target.value)} placeholder="Referral, Cold, etc." className="mt-1 bg-[#050508] border-[#1E1E26] text-[#FFFFFF]" /></div>
          <div><Label className="text-[#A0A0A0] text-xs">Estimated Value</Label><Input type="number" value={form.estimated_value || ''} onChange={e => set('estimated_value', parseFloat(e.target.value) || 0)} className="mt-1 bg-[#050508] border-[#1E1E26] text-[#FFFFFF]" /></div>
          ...
          <div><Label className="text-[#A0A0A0] text-xs">Priority</Label>
            <Select value={form.priority || 'Medium'} onValueChange={v => set('priority', v)}>
              <SelectTrigger className="mt-1 bg-[#050508] border-[#1E1E26] text-[#FFFFFF]"><SelectValue /></SelectTrigger>
              <SelectContent className="bg-[#0D0D12] border-[#1E1E26]">{PRIORITIES.map(p => <SelectItem key={p} value={p} className="text-[#FFFFFF] focus:bg-[#161620]">{p}</SelectItem>)}</SelectContent>
            </Select>
          </div>
          ...
          <div><Label className="text-[#A0A0A0] text-xs">Last Contacted</Label><Input type="date" value={form.last_contacted_date || ''} onChange={e => set('last_contacted_date', e.target.value)} className="mt-1 bg-[#050508] border-[#1E1E26] text-[#FFFFFF]" /></div>
          <div><Label className="text-[#A0A0A0] text-xs">Next Follow-Up</Label><Input type="date" value={form.next_follow_up_date || ''} onChange={e => set('next_follow_up_date', e.target.value)} className="mt-1 bg-[#050508] border-[#1E1E26] text-[#FFFFFF]" /></div>
          <div className="sm:col-span-2"><Label className="text-[#A0A0A0] text-xs">Services Needed</Label><Input value={form.services_needed || ''} onChange={e => set('services_needed', e.target.value)} placeholder="New website, SEO, maintenance..." className="mt-1 bg-[#050508] border-[#1E1E26] text-[#FFFFFF]" /></div>
        ...
        <DialogFooter>
          ...
          <Button onClick={handleSave} disabled={saving || !form.company_name} className="bg-[#00F0FF] hover:bg-[#00C8D6] text-white">{saving ? 'Saving...' : 'Save Lead'}</Button>
        </DialogFooter>
      </DialogContent>
    </Dialog>
  );
